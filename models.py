import enum
from sqlalchemy import Column, Integer, String, Date, DateTime, Numeric, ForeignKey,Boolean, Enum as SQLEnum, Text
...
class Apartment(Base):
    ...
    telegram = Column(String, nullable=True) # Додаємо це поле
    ...
    images   = relationship("ApartmentImage", back_populates="apartment", cascade="all, delete-orphan",order_by="ApartmentImage.is_main.desc()")
...
class ApartmentImage(Base):
    ...
    # is_main — головне фото (показується в картці списку)
...
class Contract(Base):
    ...
    total_sum=Column(Numeric, nullable=True)
    apartment=relationship("Apartment")
