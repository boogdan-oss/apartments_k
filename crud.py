...
def create_person(db: Session, person: schemas.PersonCreate):
    ...
    if person.role == models.UserRole.owner:
        ...
        db.add(db_client)
        db.commit()
    elif person.role == models.UserRole.realtor:
        # Перевіряємо, чи вказав користувач комісію (це обов'язкове поле в БД)
        if person.commission_rate is None:
            db.delete(db_person) 
            db.commit()
            raise HTTPException(status_code=400, detail="Для рієлтора обов'язково потрібна ставка комісії (commission_rate)")
